...
export function Hero({ scrollToSection }: HeroProps) {
  const line1 = 'Where Every Meal'.split(' ');
  const line2 = 'Is a Statement'.split(' ');
  ...
  return (
    ...
    >
      ...
      <img
        ...
        onLoad={e => { setTimeout(() => { (e.target as HTMLImageElement).style.transform = 'scale(1)'; }, 100); }}
      />
      ...
      <div style={{ position: 'relative', zIndex: 10, textAlign: 'center', padding: '0 24px', maxWidth: 860, margin: '0 auto' }}>
        ...
        <p className="fb gold" style={{ fontSize: 12, fontWeight: 600, letterSpacing: '0.22em', textTransform: 'uppercase', marginBottom: 20, animation: 'fadeIn 0.8s ease 0.3s both' }}>
          Greda Estate, Teshie · Accra, Ghana
        </p>

        {/* Animated headline */}
        <h1 className="fd hero-h1 glow" style={{ fontSize: 'clamp(40px, 8vw, 90px)', fontWeight: 700, lineHeight: 1.08, color: '#fff', marginBottom: 22 }}>
          <span style={{ display: 'block' }}>
            {line1.map((word, i) => (
              <span key={i} className="word-animate" style={{ animationDelay: `${0.4 + i * 0.12}s`, marginRight: '0.25em' }}>
                {word}
              </span>
            ))}
          </span>
          <span style={{ display: 'block' }}>
            {line2.map((word, i) => (
              <span key={i} className="word-animate" style={{ color: '#F97316', animationDelay: `${0.7 + i * 0.12}s`, marginRight: '0.25em' }}>
                {word}
              </span>
            ))}
          </span>
        ...
        <div className="bounce" style={{ marginTop: 60, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 5, opacity: 0.5, animation: 'fadeIn 0.8s ease 1.5s both' }}>
